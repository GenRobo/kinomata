...
:run_app
echo --- Running: "%BUILD_DIR%\%BUILD_CONFIG%\kinomata_tests.exe" %APP_ARGS% ---
if not exist "%BUILD_DIR%\%BUILD_CONFIG%\kinomata_tests.exe" (
    echo Error: Executable not found at "%BUILD_DIR%\%BUILD_CONFIG%\kinomata_tests.exe"
    exit /b 1
)
"%BUILD_DIR%\%BUILD_CONFIG%\kinomata_tests.exe" %APP_ARGS%
exit /b %errorlevel%
